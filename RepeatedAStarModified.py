...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 23 22:32:47 2021
...
class RepeatedAStarSearchModified:
    x_cord = [0, 1, 0, -1]
    y_cord = [1, 0, -1, 0]
    childs=[]
    ...
    def aStar(self,x,y,matrix):
        ...
        while self.queue.__len__() > 0:
            ...
            for i in range(0,4):
                ...
                x_temp = RepeatedAStarSearchModified.x_cord[i] + curr_x
                y_temp = RepeatedAStarSearchModified.y_cord[i] + curr_y
    ...
    def findPath(self):
        ...
        # print("Path is:")
        # print(self.Finaltrajectory_length)
        # print(findpath[::-1])
        ...
        for tuplecoord in findpathrev:
            ...
            if self.matrix[coordx][coordy]==0:
                ...
                for i in range(0,4):
                    x_temp = RepeatedAStarSearchModified.x_cord[i] + coordx
                    y_temp = RepeatedAStarSearchModified.y_cord[i] + coordy
                    if (x_temp >=0 and x_temp < self.size and y_temp >=0 and y_temp <self.size):
                        self.discovered_grid[x_temp][y_temp]=self.matrix[x_temp][y_temp]
                        if(self.discovered_grid[x_temp][y_temp]==1 and ((x_temp,y_temp) not in findpathrev)):
                ...
                if(neighborCount > 0):
                    #print(x_temp,y_temp)
                    recallStack.append((coordx,coordy,tracnt-1))
                    
        if(flag is True):
            if(len(recallStack)>0):
                #print(recallStack)
                restartX,restartY,trajlen=recallStack.pop()
                #print(restartX,restartY)
                self.Finaltrajectory_length=self.Finaltrajectory_length+trajlen
                res=self.aStar(restartX,restartY,self.discovered_grid)
            else:
                res=False
            ...
            self.Finaltrajectory_length=self.Finaltrajectory_length+tracnt-1
        return res
    ...
    def solve1(self):
        ...
        return matrix_len,discovered_len,self.Finaltrajectory_length,time_in_seconds,nodesexp,finalSolvable
